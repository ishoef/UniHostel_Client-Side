    ...
    <section className="bg-gradient-to-b from-orange-50 to-white py-16 px-4 text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
        {steps.map((step) => (
          <div
            key={step.id}
            className="bg-white rounded-3xl shadow-lg p-6 hover:shadow-2xl transform hover:-translate-y-2 transition duration-300"
          >
            <div className="relative flex items-center justify-center mb-6">
              <div className="bg-gradient-to-r from-orange-500 to-pink-500 rounded-full p-5 shadow-md">
                {step.icon}
              </div>
              <span className="absolute -top-2 -right-2 bg-black text-white rounded-full text-xs px-2 py-1 shadow">
                {step.id}
              </span>
            </div>
